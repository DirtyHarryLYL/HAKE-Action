...
class BatchLoader(object):
    ...
    def __init__(self, params, result):
        ...
        # param of whether or not use inner wts
        self.use_10v_inner_wts = params['use_10v_inner_wts']
        self.use_117_inner_wts = params['use_117_inner_wts']
        self.use_pvp_inner_wts = params['use_pvp_inner_wts']
    ...
    def load_next_image(self):
        ...
        anns = load_annotation(index, self.pascal_root, self.im_shape, self.use_10v_inner_wts, self.use_117_inner_wts, self.use_pvp_inner_wts)
...
def load_annotation(index, pascal_root, im_shape, use_10v_inner_wts, use_117_inner_wts, use_pvp_inner_wts):
    """
    ...
    for i in range(len(label)):
        if label[i] == 1:
            verb_no = get_verb_no(pascal_root, i+1)
    ...
    if use_117_inner_wts == True:
        with open(pascal_root+'verb_list_wts_hico_pvp55.txt', 'r') as f_v:
            verb_list_wts_t = [float(line.rstrip('\n')) for line in f_v]
    else:
        verb_list_wts_t = np.ones(117, dtype=np.float)
    verb_list_wts = np.zeros(117, dtype=float)
    for i in range(len(verb_list_wts)):
        if verb_list[i] == 1:
            verb_list_wts[i] = verb_list_wts_t[i]
        else:
            verb_list_wts[i] = 1.0
    ...
    for i in range(len(label)):
        if label[i] == 1:
            object_no = get_object_no(pascal_root, i+1)
    ...
    f.close()          
    with open(pascal_root+'pos_weights.txt', 'r') as f1:
    ...
    for i in range(10):
        hp_list[int(i)] = int(data.getElementsByTagName('hp'+str(i))[0].childNodes[0].data)
 
    hp_list_wts = np.zeros(10, dtype=np.float)
    if use_10v_inner_wts == True:
        with open(pascal_root+'hp_list_wts_hico_pvp55.txt', 'r') as f3:
            hp_wts = [float(line.rstrip('\n')) for line in f3]
    else:
        hp_wts = np.ones(10, dtype=np.float)
    for i in range(10):
        if hp_list[i] == 1:
    ...
    for i in range(6):
        pvp_ankle2[int(i)] = int(data.getElementsByTagName('ankle'+str(i))[0].childNodes[0].data)
    pvp_ankle2_wts = np.zeros(6, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_ankle2_wts_t = [6.7, 18.5, 30.5, 21.5, 14.3, 1.2]
    else:
        pvp_ankle2_wts_t = np.ones(6, dtype=np.float)
    for i in range(6):
        if pvp_ankle2[i] == 1:
            pvp_ankle2_wts[i] = pvp_ankle2_wts_t[i]
        else:
            pvp_ankle2_wts[i] = 1.0

    pvp_knee2 = np.zeros(6, dtype=np.int32)
    for i in range(6):
        pvp_knee2[int(i)] = int(data.getElementsByTagName('knee'+str(i))[0].childNodes[0].data)
    pvp_knee2_wts = np.zeros(6, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_knee2_wts_t = [20.3, 32.2, 14.4, 18.0, 12.4, 0.5]
    else:
        pvp_knee2_wts_t = np.ones(6, dtype=np.float)
    for i in range(6):
        if pvp_knee2[i] == 1:
            pvp_knee2_wts[i] = pvp_knee2_wts_t[i]
        else:
            pvp_knee2_wts[i] = 1.0

    pvp_hip = np.zeros(3, dtype=np.int32)
    for i in range(3):
        pvp_hip[int(i)] = int(data.getElementsByTagName('hip'+str(i))[0].childNodes[0].data)
    pvp_hip_wts = np.zeros(3, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_hip_wts_t = [5.8, 18.8, 1.4]
    else:
        pvp_hip_wts_t = np.ones(3, dtype=np.float)
    for i in range(3):
        if pvp_hip[i] == 1:
            pvp_hip_wts[i] = pvp_hip_wts_t[i]
        else:
            pvp_hip_wts[i] = 1.0
    
    pvp_hand2 = np.zeros(23, dtype=np.int32)
    for i in range(23):
        pvp_hand2[int(i)] = int(data.getElementsByTagName('hand'+str(i))[0].childNodes[0].data)
    pvp_hand2_wts = np.zeros(23, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_hand2_wts_t = [3.4, 23.2, 14.9, 14.4, 22.7, 27.7, 23.3, 21.0, 35.0, 27.1, 17.9, 25.0, 26.6, 23.5, 18.8, 20.2, 21.1, 18.2, 20.6, 20.9, 28.9, 26.7, 3.7]
    else:
        pvp_hand2_wts_t = np.ones(23, dtype=np.float)
    for i in range(23):
        if pvp_hand2[i] == 1:
            pvp_hand2_wts[i] = pvp_hand2_wts_t[i]
        else:
            pvp_hand2_wts[i] = 1.0
    
    pvp_shoulder2 = np.zeros(5, dtype=np.int32)
    for i in range(5):
        pvp_shoulder2[int(i)] = int(data.getElementsByTagName('shoulder'+str(i))[0].childNodes[0].data)
    pvp_shoulder2_wts = np.zeros(5, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_shoulder2_wts_t = [15.6, 17.2, 17.7, 18.0, 0.4]
    else:
        pvp_shoulder2_wts_t = np.ones(5, dtype=np.float)
    for i in range(5):
        if pvp_shoulder2[i] == 1:
            pvp_shoulder2_wts[i] = pvp_shoulder2_wts_t[i]
        else:
            pvp_shoulder2_wts[i] = 1.0
    
    pvp_head = np.zeros(12, dtype=np.int32)
    for i in range(12):
        pvp_head[int(i)] = int(data.getElementsByTagName('head'+str(i))[0].childNodes[0].data)
    pvp_head_wts = np.zeros(12, dtype=np.float)
    if use_pvp_inner_wts == True:
        pvp_head_wts_t = [16.8, 8.5, 39.8, 17.1, 25.6, 16.7, 25.6, 21.3, 21.0, 28.4, 18.4, 1.2]
    else:
        pvp_head_wts_t = np.ones(12, dtype=np.float)
    for i in range(12):
        if pvp_head[i] == 1:
            pvp_head_wts[i] = pvp_head_wts_t[i]
        else:
            pvp_head_wts[i] = 1.0
...
# Param: hoi-no
# Output: verb_id
def get_verb_no(pascal_root, hoi_no):
    verb_name = ''
    with open(osp.join(pascal_root, 'info/hoi.txt'), 'r') as f:
        for line in f.readlines():
            if int(line.split()[0]) == hoi_no:
                verb_name = line.split()[2]
                break
    with open(osp.join(pascal_root, 'info/verb.txt'), 'r') as f1:
...
# Param: hoi-no
# Output: object_id
def get_object_no(pascal_root, hoi_no):
    object_name = ''
    with open(osp.join(pascal_root, 'info/hoi.txt'), 'r') as f:
        for line in f.readlines():
            if int(line.split()[0]) == hoi_no:
                ...
                break
    with open(osp.join(pascal_root, 'info/object.txt'), 'r') as f1:
        for line in f1.readlines():
