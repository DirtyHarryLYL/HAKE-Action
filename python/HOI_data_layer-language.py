...
hoi_matrix = np.zeros((600,2304))
from pvp_55_2304_from76_sum_averg_matrix import pvp_55_matrix
...
class HOIDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        #language_pvp_55, language_hoi, language_obj
        top[34].reshape(self.batch_size, 55, 2304)        
        top[35].reshape(self.batch_size, 600, 2304)
        top[36].reshape(self.batch_size, 80, 768)
        # detected object list of 80, if == 1, exists
        top[37].reshape(self.batch_size, 80)
    ...
    def forward(self, bottom, top):
        ...
        for itt in range(self.batch_size):
            ...
            top[34].data[itt,...] = language_pvp_55
            top[35].data[itt,...] = language_hoi
            top[36].data[itt,...] = language_obj
            top[37].data[itt,...] = det_object_list
#print top[16].data
...
class BatchLoader(object):
    ...
    def __init__(self, params, result):
        ...
        # load matrix
        pascal_root = self.pascal_root
        ## pvp_55_2304
        self.pvp_55_matrix = np.array(pvp_55_matrix)

        ## hoi_600_2304
        self.hoi_matrix = np.array(hoi_matrix)

        ## obj_80_768
        self.obj_matrix = np.array(obj_matrix)
      
        ## label_wts
        with open(pascal_root+'pos_weights.txt', 'r') as f1:
            self.pos_wts = [float(line.rstrip('\n')) for line in f1]
        with open(pascal_root+'neg_weights.txt', 'r') as f2:
            self.neg_wts = [float(line.rstrip('\n')) for line in f2]
        ## hp_list_wts
        with open(pascal_root+'hp_list_wts.txt', 'r') as f3:
            self.hp_wts = [float(line.rstrip('\n')) for line in f3]
    ...
    def load_next_image(self):
       ...
       # Did we finish an epoch?
        ...
        det_object_list = anns['det_object_list']
        self._cur += 1
        # self.transformer.preprocess(im): change RGB to BGR
 
        #load matrix
        language_pvp_55 = self.pvp_55_matrix
        language_hoi = self.hoi_matrix
        language_obj = self.obj_matrix
        
        #load wts
        label_wts = np.zeros(600, dtype=np.float)
        for i in range(600):
            if label[i] == 1:
                label_wts[i] = math.log10(self.pos_wts[i])*10
            elif label[i] == 0:
                label_wts[i] = 1
            else:
                label_wts[i] = 0
        ...
        for i in range(10):
            if hp_list[i] == 1:
                hp_list_wts[int(i)] = self.hp_wts[i]
            else:
                hp_list_wts[i] = 1 
...
def load_annotation(index, pascal_root, im_shape):
    ...
    for i in range(len(label)):
        ...
        if label[i] == -1:
            ...
            if object_list[object_no-1] != 1:
                object_list[object_no-1] = -1
    object_list_wts = np.ones(80, dtype=float)
    # for i in range(len(object_list_wts)):
    #     object_list_wts[i] = 1.0
            
    det_object_list = np.zeros(80, dtype=np.int32)
    ...
    for ix, obj in enumerate(objs):
        cls = int(float(get_data_from_tag(obj, "name")))
        hico_cls = detection2hico[cls]
        det_object_list[hico_cls-1] = 1
    #print "label:", label
    f.close()          
    ...
    pvp_ankle2_wts = np.ones(6, dtype=np.float)
    # for i in range(6):
    ...
    pvp_knee2_wts = np.ones(6, dtype=np.float)
    # for i in range(6):
    ...
    pvp_hip_wts = np.ones(3, dtype=np.float)
    # for i in range(3):
    ...
    pvp_hand2_wts = np.ones(23, dtype=np.float)
    # for i in range(23):
    ...
    pvp_shoulder2_wts = np.ones(5, dtype=np.float)
    # for i in range(5):
    ...
    pvp_head_wts = np.ones(12, dtype=np.float)
    # for i in range(12):
    #     pvp_head_wts[int(i)] = 1.0

    pvp_wts = np.ones(6, dtype = np.float)
    # for i in range(6):
    #     pvp_wts[int(i)] = 1.0
