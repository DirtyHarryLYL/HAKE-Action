...
class HOIDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        top[34].reshape(self.batch_size, 6) # pvp_cont
...
def load_annotation(index, pascal_root, im_shape):
    ...
    pvp_cont = np.zeros(6, dtype=np.int32) # 6 pvps, maybe 10 pvps later
    # with open(pascal_root+'pvp_cont.txt', 'r') as f4:
    #     pvp_cont = [float(line.rstrip('\n')) for line in f4]
    for i in range(6):
        if i == 0:
            pvp_cont[i] = 0
        else:
