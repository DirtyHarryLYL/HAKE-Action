...
__global__ void SigmoidCrossEntropyLossForwardGPU(const int nthreads,
          const Dtype* input_data, const Dtype* weights, const Dtype* target, Dtype* loss) {
  CUDA_KERNEL_LOOP(i, nthreads) {
    const int target_value = static_cast<int>(target[i]);
    loss[i] = (input_data[i] * (target[i] - (input_data[i] >= 0)) -
        log(1 + exp(input_data[i] - 2 * input_data[i] * (input_data[i] >= 0))))*weights[i];
  }
}
...
template <typename Dtype>
__global__ void SigmoidCrossEntropyLossWeightGPU(const int count,
   const Dtype* weights, const Dtype* target, Dtype* reference) {
  CUDA_KERNEL_LOOP(index, count) {
    const int target_value = static_cast<int>(target[index]);
      reference[index] *= weights[index];
  }
}
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  SigmoidCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
      CAFFE_CUDA_NUM_THREADS>>>(count, input_data, weights, target, loss_data);
...
void SigmoidCrossEntropyLossLayer<Dtype>::Backward_gpu(
  ...
  if (propagate_down[0]) {
    ...
    if (has_weight_positive_) {
      // NOLINT_NEXT_LINE(whitespace/operators)
      SigmoidCrossEntropyLossWeightGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
        CAFFE_CUDA_NUM_THREADS>>>(count, weights, target, bottom_diff);
    }
    if (has_ignore_label_) {
      // NOLINT_NEXT_LINE(whitespace/operators)
      SigmoidCrossEntropyLossIgnoreGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
