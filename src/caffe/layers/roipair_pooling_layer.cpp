...
void ROIPairPoolingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ROIPairPoolingParameter roi_pool_param = this->layer_param_.roipair_pooling_param();
...
template <typename Dtype>
void ROIPairPoolingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
...
template <typename Dtype>
void ROIPairPoolingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
          ...
          if (!is_empty) {
            for (int h = hstart; h < hend; ++h) {
              for (int w = wstart; w < wend; ++w) {
                ...
                if (batch_data[index] > top_data[pool_index] and (in_roi1 or in_roi2)) {
...
template <typename Dtype>
void ROIPairPoolingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(ROIPairPoolingLayer);
#endif

INSTANTIATE_CLASS(ROIPairPoolingLayer);
REGISTER_LAYER_CLASS(ROIPairPooling);
